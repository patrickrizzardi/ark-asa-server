...
- [ ] `the-center` boots on dell and advertises for join with `ARK_CLUSTER_ID` set (cluster args present in the launch log line)
  - Evidence: NOT VERIFIED — requires a live dell boot, which this executor did not perform. Static verification only (re-run 2026-07-06 post-review-fix): `docker compose --env-file <test env with ARK_CLUSTER_ID set> config` shows `ARK_CLUSTER_ID: my-secret-cluster` reaching the `the-center` service environment (now via a `:-` default, not the old `:?`-required form — see the volume/AC below), and the entrypoint flags-construction logic was simulated in bash (`[[ -n "$ARK_CLUSTER_ID" ]] && flags="${flags} -clusterid=... -ClusterDirOverride=..."`), producing `-clusterid=my-secret-cluster -ClusterDirOverride=/home/container/arkserver/ShooterGame/Saved/clusters` when set. A boot-time charset guard (`entrypoint.sh:464-467`, `[A-Za-z0-9._-]` only) was added and verified to reject `bad id; rm -rf` and accept `my-secret-cluster.01`. The actual "boots on dell and advertises" behavior is unchecked. **STILL UNCHECKED after the 2026-07-06 real dell boot** (see the "Live dell boot verification" entry below) — that session ran with dell's real `.env` `ARK_CLUSTER_ID` UNSET (a single-server regression boot, proving the sibling empty-clusterid AC instead — see below), not SET, so it does not satisfy this AC's literal wording. A live boot with `ARK_CLUSTER_ID` actually set is still pending; that is naturally exercised in Phase 3 (two servers sharing one clusterid).
...
**Live dell boot verification (2026-07-06)** — a real SSH session against the `dell` host,
`ark-the-center` container rebuilt from the pushed Phase 1+2 commits (`7cc1960`) and restarted, with
dell's real `.env` `ARK_CLUSTER_ID` UNSET (a single-server regression boot — not yet a
cluster-id-SET boot; that's Phase 3's job with two servers sharing one clusterid):
- Boot log: `[UE log] Commandline: "TheCenter_WP?listen?SessionName=Silica Valley PvP | 6-Man | 15x
  H/T | 300xB?Port=7777?MaxPlayers=40?RCONEnabled=True?RCONPort=27020?ServerAdminPassword=<redacted>"
  -log -WinLiveMaxPlayers=40 -BattlEye -mods=955333,942249,928597,947033,930684` — confirmed via a
  real `docker logs`, NOT a bash simulation: no `-clusterid=` and no `-ClusterDirOverride=` token
  anywhere. `[UE log] Server: "..." has successfully started! Full Startup: 82.17 seconds.`
  `docker inspect ark-the-center` → `Status: running | Restarts: 0 | ExitCode: 0` (clean boot, no
  crash-loop). Host `ss -lun` shows `0.0.0.0:7777` UDP bound; `ss -lnt` shows `27020` TCP LISTEN.
- `docker exec ark-the-center ls -la .../Saved/clusters` → confirmed a symlink to
  `/home/container/cluster-data` (Phase 1's shallow-mount fix), live and working on the real dell
  filesystem.
- This closes the two dell-gated Phase 1 ACs it actually covers (ticked above with this evidence)
  and leaves the `ARK_CLUSTER_ID`-SET AC honestly unchecked — this round tested the empty-clusterid
  path, not the set path (see that AC's note).
...
**Acceptance criteria**:
- [x] `the-center` boots alone on dell with loot (Game.ini), shop catalog, Permissions, and GUS tuning all applied (single-server regression intact)
  - Evidence: CONFIRMED LIVE (2026-07-06 real dell boot — see Phase 2's "Live dell boot verification" entry below): `docker logs` on the rebuilt `ark-the-center` (commit `7cc1960`) shows `[entrypoint] GameUserSettings.ini deployed from repo canonical...`, `[entrypoint] Deploying AsaApi + plugins from image to volume…`, `[entrypoint] Deployed ArkShop config.json from repo seed...`, `[entrypoint] Deployed Permissions config.json from repo seed...`, `[entrypoint] ArkShop DB config injected...`, `[entrypoint] Permissions DB config injected...`, then `[UE log] Server: "..." has successfully started! Full Startup: 82.17 seconds.` — every deploy step fired and the server booted clean (`docker inspect` → `Restarts: 0, ExitCode: 0`). Static proxies (bash -n, compose config, buildx check) remain green as a secondary check.
- [x] `${WindowsServer}` is a real directory (NOT a symlink); both `Game.ini` and `GameUserSettings.ini` inside it are real regular files (`stat`/`readlink` evidence), copied from `config/` each boot
  - Evidence: CONFIRMED LIVE (2026-07-06): `docker exec ark-the-center stat .../Saved/Config/WindowsServer` → type `directory`; `readlink` on it exits 1 (not a symlink). `stat .../WindowsServer/Game.ini` → regular file, 1243410 bytes. `stat .../WindowsServer/GameUserSettings.ini` → regular file, 14405 bytes. This boot ran against dell's EXISTING `ark-game` volume (up 17hrs on the OLD pre-Phase-2 symlinked model immediately before this deploy), so it is also the dirty-volume-transition test — the old whole-dir symlink was cleanly replaced by a real directory with no dangling links (see the Quality-gate dirty-volume item below). Prior static-sim evidence (sandbox run of the verbatim main() block) stands as the pre-boot corroboration.
- [x] `GameUserSettings.ini` `SessionName` matches `${SESSION_NAME}` (injected, not the canonical's value)
  - Evidence: CONFIRMED LIVE (2026-07-06): `docker exec ark-the-center grep SessionName .../WindowsServer/GameUserSettings.ini` → `SessionName=Silica Valley PvP | 6-Man | 15x H/T | 300xB` — matches the env-configured session name (also visible in the boot log's `SessionName=` launch-string token and the `[entrypoint] GameUserSettings.ini deployed...` log line). Prior static-sim evidence (verbatim `seed_gus()` run against the real CRLF canonical) stands as the pre-boot corroboration.
- [x] ArkShop + Permissions `config.json` are real per-server files in their plugin dirs (NOT symlinks); `config/permissions.config.json` exists as a secret-free tracked seed
  - Evidence: CONFIRMED LIVE (2026-07-06): both plugin configs located via `find` at `ShooterGame/Binaries/Win64/ArkApi/Plugins/{ArkShop,Permissions}/config.json`; both are regular files (`readlink` exits 1 on each, not symlinks). A repo-wide `find .../Saved/Config -type l` and `find .../ArkApi/Plugins -type l` both returned EMPTY on the real dell container — no stray symlinks anywhere in the config or plugin-config paths (the only symlinks present anywhere in the entrypoint's `ln -s` inventory are the intentional cluster-data link and 2 unrelated pre-existing steamclient.so links, grep-confirmed against `entrypoint.sh`). Seed half was already VERIFIED prior (`config/permissions.config.json` tracked, captured from the real image default, creds blanked).
- [x] **DB inject still works on the real-file path**: `jq .Mysql` on the ArkShop config AND `jq '{UseMysql,MysqlHost,MysqlUser,MysqlPass,MysqlDB,MysqlPort}'` on the Permissions config shows the injected host/user/db — proves `_inject_mysql_block`'s symlink-resolution removal didn't break the write (the silent-DB-less-boot risk)
  - Evidence: CONFIRMED LIVE (2026-07-06, criterion wording per FRAGO 003 — the Permissions plugin's real schema is FLAT, root-level Mysql* keys): on the real dell container, `jq .Mysql` on the ArkShop config.json shows `{"UseMysql": true, "MysqlHost": "mariadb", "MysqlUser": "arkshop", "MysqlPass": "<redacted>", "MysqlDB": "arkshop", "MysqlPort": 3306}` (nested schema); `jq '{UseMysql,MysqlHost,MysqlUser,MysqlPass,MysqlDB,MysqlPort}'` on the Permissions config.json shows the identical injected values at the JSON root (flat schema) — `{"UseMysql": true, "MysqlHost": "mariadb", "MysqlUser": "arkshop", "MysqlPass": "<redacted>", "MysqlDB": "arkshop", "MysqlPort": 3306}`. Both real per-server files on the real running container, not a sandbox simulation — this is the critical AC from FRAGO 003, now confirmed live in production. (Real password value redacted here per this doc's own secret-handling constraint; the actual value was seen in the live `jq` output during the dell session.)
...
- [x] Dirty-volume transition is clean: booting Phase 2 on a volume that still holds the OLD symlinked `config.json` (from a pre-Phase-2 boot) correctly replaces it with a real file (not a dangling symlink) — verify on dell's existing `ark-game` volume, not just a fresh one
  - Evidence: CONFIRMED LIVE (2026-07-06): dell's `ark-game` volume had been running the OLD pre-Phase-2 symlinked config model for 17 hours immediately before this deploy (container `ark-the-center` was up 17hrs on the old image, then rebuilt from `7cc1960` and restarted) — this real boot IS the dirty-volume-transition test this gate names, on the actual existing volume, not a fresh one. It transitioned cleanly: `WindowsServer` came up as a real directory (not a dangling symlink; `readlink` exits 1), both plugin config.json files came up as real files (`readlink` exits 1 on each), and the whole-repo `find -type l` sweep over `Saved/Config` and `ArkApi/Plugins` came back EMPTY — no dangling links, no errors, `docker inspect` shows a clean boot (`Restarts: 0, ExitCode: 0`). Prior static analysis (deploy_plugins' `rm -rf ArkApi` clean-replace; the WindowsServer transition sandbox sim) stands as the pre-boot corroboration.
...
**Live dell boot verification (2026-07-06)** — a real SSH session against the `dell` host,
`ark-the-center` container rebuilt from the pushed Phase 1+2 commits (`7cc1960`) and restarted, with
dell's real `.env` `ARK_CLUSTER_ID` unset (a single-server regression boot; see Phase 1's identical
dated entry for the shared boot-log context and cluster-dir symlink evidence):
- Boot log deploy lines all present: `GameUserSettings.ini deployed from repo canonical`,
  `Deploying AsaApi + plugins from image to volume`, `Deployed ArkShop config.json from repo seed`,
  `Deployed Permissions config.json from repo seed`, `ArkShop DB config injected`, `Permissions DB
  config injected`, followed by a successful UE start (`Full Startup: 82.17 seconds`).
- In-container `stat`/`readlink`/`find`/`grep`/`jq` checks (see the ACs above for exact evidence)
  confirm: `WindowsServer` is a real dir with real regular-file `Game.ini`/`GameUserSettings.ini`;
  the injected `SessionName` matches the env config; ArkShop + Permissions `config.json` are real
  per-server files (not symlinks) with the DB settings correctly injected in both the nested
  (ArkShop) and flat (Permissions) schemas; and this ran against dell's volume that had been on the
  OLD pre-Phase-2 symlinked model for the prior 17 hours, so it doubles as the dirty-volume
  transition test — which passed clean (no dangling symlinks anywhere in `Saved/Config` or
  `ArkApi/Plugins`).
- **Not covered by this session** (left honestly unchecked/unclaimed): no in-game verification was
  performed (no player joined); no cluster transfer was attempted (single-server boot only, and
  that's Phase 3's job); the `ARK_CLUSTER_ID`-SET boot path was not exercised this round (see
  Phase 1's sibling AC note).
- **Housekeeping note (not a gate item, not a blocker):** `plugins-config/` still exists as an
  orphaned directory on dell's HOST filesystem — it is not git-tracked (the repo-tracked
  `plugins-config/.gitkeep` was already `git rm`'d in this phase's Step 8), just a leftover from
  before the shared bind was removed. Suggest `rm -rf ~/ark-asa-server/plugins-config` on dell
  whenever convenient; it has no effect on the running stack.

**Phase Review Gates**:
- [x] code-reviewer: clean — 2026-07-06 (round 3, final pass — clean after prior rounds closed all should-fix/BLOCKER findings)
- [x] rules-compliance-reviewer: clean — 2026-07-06 (round 3, final pass)
- [x] plan-adherence-verifier: clean — 2026-07-06 (round 3, final pass; no distinct plan-adherence-verifier finding was logged in any of rounds 1-2 — clean throughout)
- [x] acceptance-verifier: met — 2026-07-06 (directly attested running across all 3 rounds; met/clean on this final round)
- [x] design-compliance-reviewer: clean — 2026-07-06 (round 3, final pass; no distinct design-compliance-reviewer finding was logged in any of rounds 1-2 — covered by the same review-fleet pass as rules-compliance-reviewer, no separate design-side finding surfaced)
- [x] Committed: `7cc1960`
