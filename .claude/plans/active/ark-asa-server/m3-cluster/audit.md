...
- **Addendum — pdb/exe version-matching, resolved (2026-07-06, same-day follow-up, Opus advisory + direct source verification).** A separate concern was raised: `pdb_ok()` (entrypoint.sh) only checks presence+size, never that the pdb corresponds to the currently-installed build — could a future `UPDATE_ON_BOOT=1` silently pair a stale pdb with a newer exe? **Read AsaApi's actual source** ([`ArkBaseApi.cpp`](https://github.com/ArkServerApi/AsaApi/blob/master/AsaApi/Core/Private/Ark/ArkBaseApi.cpp), the pdb-load path around `Failed to read pdb`): AsaApi SHA-256s the pdb file itself purely as an offset-cache key (checks a community CDN for a pre-computed cache by that hash, else parses the pdb's own symbol table directly) — **it never cross-checks the exe's build ID or debug-directory GUID against the pdb at all.** So no version-matching safety net exists in AsaApi itself, confirmed, not assumed. However: **real production evidence already answers whether this matters in practice** — The Center has been running a `Jun-21`-dated pdb against a `Jul-5`-dated exe (a genuine build mismatch) successfully this whole time, and Genesis just replicated the identical pairing (same copied pdb) with all 5 AsaApi mods loading clean. **Decision: no code change.** Building a buildid-stamp/mismatch-detection feature was considered and rejected — it would (a) solve a problem with no demonstrated real-world impact (the exact mismatch class already works in production), and (b) its only available remedy on a detected mismatch (re-run steamcmd validate) is the same provably-futile action that caused this whole incident, so it would just add a second trigger for the identical crash-loop pathology this deferral's own fix (§ crashloop hold-and-scream below) just closed. `UPDATE_ON_BOOT` already defaults to `0` (attended deploys only) — left as-is; no new mitigation needed since the risk this addendum evaluated is not demonstrated to exist. Phase 3's own remaining RCON/economy ACs (SetPoints/GetPoints across both maps) are the real functional proof this pairing works, not a separate synthetic check.
