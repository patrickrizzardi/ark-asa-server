...
const itemIndex = beacon.buildIndex(beacon.parseTsv(join(refDir, 'engrams.tsv')), (r) => r.class_string);
const resolve = beacon.makeResolver(itemIndex, misses); // bare class string (identity format)
const resolveMany = (labels: string[]): string[] => labels.map((l) => resolve(l));
...
// --- main ---
const crates = beacon.parseTsv(join(refDir, 'loot_containers.tsv'));
